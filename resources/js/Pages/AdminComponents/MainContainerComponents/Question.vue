...
const typeSort = [
    ...
    {id: 3, category: 'question', name: 'По вопросу', type: 2},
    {id: 4, category: 'question', name: 'По вопросу', type: 1},
...
const contentComputed = computed(() => {
    ...
    } else if (sort.value.category === 'question') {
        filtered = [...filtered].sort((a, b) => {
            const result = a.question.localeCompare(b.name);
            return sort.value.type === 1 ? result : -result;
        });
...
const create = () => {
    emit('createNew', 'Question')
}
const update = (item) => {
    emit('update', item, 'Question')
    openMenuIndex.value = false;
}
const del = (item) => {
    emit('del', item, 'Question')
    openMenuIndex.value = false;
}
...
<template>
    ...
    <div class="w-full h-max relative flex flex-col gap-4">
        <h2 class="ms-8 font-bold text-4xl w-full text-gray-800">Карточки "Внимание, вопрос!"</h2>
        <div class="w-full h-max bg-white rounded-xl px-8 py-12 mt-10">
            <div class="flex justify-between border-b-2 border-gray-200 pb-6">
                <div
                    class="flex w-max cursor-pointer hover:bg-fuchsia-200 justify-center items-center gap-2 rounded-full px-4 bg-gray-200"
                    @click="create">
                    ...
                    <p class="text-sm -ms-1 -mb-0.5 font-medium">Добавить карточку "Внимание, вопрос!"</p>
                    <ChevronUpIcon
                        class="col-start-1 row-start-1 size-4 mt-0.5 self-center justify-self-end text-gray-500 sm:size-4 rotate-180"
                        aria-hidden="true"/>
                </div>

                <div :class="activeRight ? 'w-3/12 g' : 'w-7/12'"
                        ...
                        <ListboxButton
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <svg v-if="sort.type === 1" width="20px" height="20px" viewBox="0 0 24 24" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M5.93866 9.12287L12 3.06152L18.0613 9.12287" stroke="black"
                                          stroke-width="1.6"
                                          stroke-linecap="round" stroke-linejoin="round" class="my-path"></path>
                                    <path d="M12 21.0615L12 3.06152" stroke="black" stroke-width="1.6"
                                ...
                                <svg v-else width="20px" height="20px" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <path d="M18.0613 15L12 21.0613L5.93866 15" stroke="black" stroke-width="1.6"
                                          stroke-linecap="round" stroke-linejoin="round" class="my-path"></path>
                                    <path d="M12 3.06152L12 21.0615" stroke="black" stroke-width="1.6"
                                          stroke-linecap="round"
                                          class="my-path"></path>
                                </svg>
                            </div>
            ...
            <div class="mt-10">
                <div v-if="content === null"
                     class="inset-0 flex justify-center items-center bg-white bg-opacity-70 z-10">
                    <svg class="animate-spin h-8 w-8 text-fuchsia-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                         viewBox="0 0 24 24">
                ...
                <div v-else class="w-full">
                    <div v-if="content.length === 0">
                        <h2 class="font-bold text-lg text-gray-600 text-center">В данный момент не создано ни одной
                            карточки "Внимание, вопрос!"</h2>
                    </div>
                    ...
                    <table v-else
                        ...
                        <tr class="bg-gray-700 text-xl">
                            <th class="roboto font-normal w-4/12 text-white rounded-ss-xl px-5 py-4">Вопрос</th>
                            <th class="roboto font-normal w-1/12 text-white px-5 py-4">Ответ</th>
                            <th class="roboto font-normal w-4/12 text-white px-5 py-4">Дополнение к ответу</th>
                            <th class="roboto font-normal w-1/12 text-white px-5 py-4">Кол-во очков</th>
                        ...
                        <tbody>
                        <tr v-for="(item, index) in contentComputed" :key="item.id"
                            :class="index % 2 === 1 ? 'bg-gray-200' : 'bg-white'" @click="activeRightBar(item)"
                            class="rounded cursor-pointer hover:bg-fuchsia-300 transition-all duration-300 ease-in-out">
                            <td :class="index + 1 === contentComputed.length ? 'rounded-es-xl' : ''"
                            ...
                            <td class="border-gray-800 px-5 py-3">
                                <div class="prose prose-lg scroll-container overflow-y-scroll mt-1 max-h-[8rem] p-2 rounded break-words max-w-none" v-html="renderMarkdown(item.answer)"></div>
                            </td>
                            <td class="border-gray-800 px-5 py-3">
                            ...
                            <td :class="index + 1 === contentComputed.length ? 'rounded-ee-xl' : ''"
                                ...
                                <!-- Dropdown -->
                                <div :class="openMenuIndex === index ? 'block' : 'hidden'"
                                     class="absolute transition-all duration-500 right-0 border border-gray-800 mt-2 w-40 bg-white rounded-md z-10">
                                    <ul class="">
