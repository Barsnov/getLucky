...
const typeSort = [
    ...
    {id: 3, category:'points', name: 'По очкам', type: 2},
    {id: 4, category:'points', name: 'По очкам', type: 1}
]
...
const contentComputed = computed(() => {
    ...
    if (searchInput.value.trim()) {
        ...
        filtered = filtered.filter(item =>
            item.points.toString().includes(search)
        );
    }
...
const create = () =>{
    emit('createNew', 'Lucky')
}
const update = (item) =>{
    emit('update', item, 'Lucky')
...
<template>
    ...
    <div class="w-full h-max relative flex flex-col gap-4">
        <h2 class="ms-8 font-bold text-4xl w-full text-gray-800">Карточки "Удачи"</h2>
        <div class="w-full h-max bg-white rounded-xl px-8 py-12 mt-10">
            <div class="flex justify-between border-b-2 border-gray-200 pb-6">
                <div class="flex w-max cursor-pointer hover:bg-fuchsia-200 justify-center items-center gap-2 rounded-full px-4 bg-gray-200" @click="create">
                    ...
                    <p class="text-sm -ms-1 -mb-0.5 font-medium">Добавить карточку "Удачи"</p>
            ...
            <div class="mt-10">
                ...
                <div v-else class="w-full">
                    <div v-if="content.length === 0">
                        <h2 class="font-bold text-lg text-gray-600 text-center">В данный момент не создано ни одной карточки "удачи"</h2>
                    </div>
                    ...
                    <table v-else class="shadow-xl rounded-xl w-full text-left border-gray-800 table-auto border-collapse">
                        ...
                        <tr class="bg-gray-700 text-xl">
                            <th class="roboto font-normal w-3/12 text-white rounded-ss-xl px-5 py-4">Кол-во очков</th>
                            <th class="roboto font-normal w-5/12 text-white px-5 py-4">Последнее изменение</th>
                            <th class="roboto font-normal w-2/12 text-white rounded-se-xl px-5 py-4"></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="(item, index) in contentComputed" :key="item.id" :class="index % 2 === 1 ? 'bg-gray-200' : 'bg-white'" @click="activeRightBar(item)" class="rounded cursor-pointer hover:bg-fuchsia-300 transition-all duration-300 ease-in-out">
                            <td :class="index + 1 === contentComputed.length ? 'rounded-es-xl' : ''" class="border-gray-800 px-5 py-3">
                            ...
                            <td :class="index + 1 === contentComputed.length ? 'rounded-ee-xl' : ''" class="text-right relative px-5 py-3">
