...
const typeSort = [
    ...
    {id: 3, category:'word', name: 'По слову', type: 2},
    {id: 4, category:'word', name: 'По слову', type: 1}
]
...
const contentComputed = computed(() => {
    ...
    // Сортировка
    if (sort.value.category === 'word') {
        filtered = [...filtered].sort((a, b) => {
            const result = a.word.localeCompare(b.name);
...
<template>
    ...
    <div class="w-full h-max relative flex flex-col gap-4">
        <h2 class="ms-8 font-bold text-4xl w-full text-gray-800">Карточки "Словодел"</h2>
            ...
            <div class="mt-10">
                ...
                <div v-else class="w-full">
                    <div v-if="content.length === 0">
                        <h2 class="font-bold text-lg text-gray-600 text-center">В данный момент не создано ни одной карточки "Словодел"</h2>
                    </div>
                    ...
                    <table v-else class="shadow-xl rounded-xl w-full text-left border-gray-800 table-auto border-collapse">
                        ...
                        <tr v-for="(item, index) in contentComputed" :key="item.id" :class="index % 2 === 1 ? 'bg-gray-200' : 'bg-white'" @click="activeRightBar(item)" class="rounded cursor-pointer hover:bg-fuchsia-300 transition-all duration-300 ease-in-out">
                            <td :class="index + 1 === contentComputed.length ? 'rounded-es-xl' : ''" class="border-gray-800 px-5 py-3">
                                <h2 class="text-lg font-bold text-gray-800">{{item.word}}</h2>
                            </td>
                            <td class="border-gray-800 px-5 py-3">
                                <div class="scroll-container flex flex-wrap gap-2 overflow-y-auto max-h-40">
                                    <h2 class="bg-gray-300 rounded-full px-3 py-1 text-sm font-bold text-gray-800" v-for="word in item.words" :key="word">{{ word }}</h2>
                                </div>
                            </td>
