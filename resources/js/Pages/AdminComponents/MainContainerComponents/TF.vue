...
<template>
    ...
    <div class="w-full h-max relative flex flex-col gap-4">
        <h2 class="ms-8 font-bold text-4xl w-full text-gray-800">Карточки "Правда или Ложь"</h2>
        <div class="w-full h-max bg-white rounded-xl px-8 py-12 mt-10">
            <div class="flex justify-between border-b-2 border-gray-200 pb-6">
                <div class="flex w-max cursor-pointer hover:bg-fuchsia-200 justify-center items-center gap-2 rounded-full px-4 bg-gray-200" @click="create">
                    ...
                    <p class="text-sm -ms-1 -mb-0.5 font-medium">Добавить карточку "Правда или Ложь"</p>
            ...
            <div class="mt-10">
                ...
                <div v-else class="w-full">
                    <div v-if="content.length === 0">
                        <h2 class="font-bold text-lg text-gray-600 text-center">В данный момент не создано ни одной карточки "Правда или Ложь"</h2>
                    </div>
                    ...
                    <table v-else class="shadow-xl rounded-xl w-full text-left border-gray-800 table-auto border-collapse">
                        ...
                        <tr class="bg-gray-700 text-xl">
                            ...
                            <th class="roboto font-normal w-1/12 text-white px-1 py-3">Ответ</th>
                            <th class="roboto font-normal w-4/12 text-white px-1 py-3">Дополнение к ответу</th>
                            <th class="roboto font-normal w-1/12 text-white px-1 py-3">Кол-во очков</th>
                            <th class="roboto font-normal w-1/12 text-white px-1 py-3">Последнее изменение</th>
                            <th class="roboto font-normal w-1/12 text-white rounded-se-xl px-5 py-3"></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="(item, index) in contentComputed" :key="item.id" :class="index % 2 === 1 ? 'bg-gray-200' : 'bg-white'" @click="activeRightBar(item)" class="rounded cursor-pointer hover:bg-fuchsia-300 transition-all duration-300 ease-in-out">
                            ...
                            <td class="border-gray-800 px-2 py-1">
                                <h2 class="text-lg font-bold text-gray-800">{{ item.answer ? 'Правда' : 'Ложь' }}</h2>
                            </td>
                            <td class="border-gray-800 px-2 py-1">
                                <div v-if="item.explanation !== null" class="scroll-container overflow-y-scroll mt-1 max-h-[8rem] p-2 rounded break-words prose prose-lg max-w-none" v-html="renderMarkdown(item.explanation)"></div>
                            ...
                            <td :class="index + 1 === contentComputed.length ? 'rounded-ee-xl' : ''" class="text-right relative px-2 py-1">
